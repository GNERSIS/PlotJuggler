...
mosaico::PullResult makePullResult()
{
  ...
  auto schema = arrow::schema(
      { arrow::field("timestamp_ns", arrow::int64()), arrow::field("value", arrow::float64()) });
...
TEST(LocalCache, WritesAndReadsArrowIpc)
{
  ...
  auto values = std::static_pointer_cast<arrow::DoubleArray>(cached.result.batches[0]->column(1));
