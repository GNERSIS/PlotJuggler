...
public:
  ...
  public:
    ...
    bool writeBatch(const std::shared_ptr<arrow::RecordBatch>& batch, QString* error = nullptr);
  ...
  explicit LocalCache(QString root_path = defaultRootPath(),
                      qint64 max_size_bytes = defaultBudget());
  ~LocalCache();
  ...
  [[nodiscard]] Writer beginWrite(const CacheKey& key, const std::shared_ptr<arrow::Schema>& schema,
